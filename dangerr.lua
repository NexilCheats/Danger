...
local Converted = {
	["_DANGER"] = Instance.new("ScreenGui");
...
Converted["_DANGER"].DisplayOrder = 3
Converted["_DANGER"].IgnoreGuiInset = true
Converted["_DANGER"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
Converted["_DANGER"].ResetOnSpawn = false
Converted["_DANGER"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Converted["_DANGER"].Name = "DANGER"
Converted["_DANGER"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "Triple-click this region to open DANGER."
...
Converted["_Open"].Parent = Converted["_DANGER"]
...
Converted["_ListButton"].Parent = Converted["_DANGER"]
...
Converted["_Notifications"].Parent = Converted["_DANGER"]
...
Converted["_TextBoxPlaceholder"].Parent = Converted["_DANGER"]
...
Converted["_FloatingButton"].Parent = Converted["_DANGER"]
...
Converted["_FloatingButtons"].Parent = Converted["_DANGER"]
...
Converted["_AddCustomModule"].Parent = Converted["_DANGER"]
...
Converted["_DroppedGunBGUI"].Parent = Converted["_DANGER"]
...
Converted["_MurdererBGUI"].Parent = Converted["_DANGER"]
...
Converted["_Menu"].Parent = Converted["_DANGER"]
...
Converted["_HubName"].Text = "DANGER"
...
Converted["_TextLabel7"].Text = "DANGER"
...
Converted["_Toggle"].Parent = Converted["_DANGER"]
...
Converted["_Modules"].Parent = Converted["_DANGER"]
...
Converted["_DropdownFrameSample"].Parent = Converted["_DANGER"]
...
Converted["_Range"].Parent = Converted["_DANGER"]
...
Converted["_Dialog"].Parent = Converted["_DANGER"]
...
Converted["_NotificationSample"].Parent = Converted["_DANGER"]
...
do -- Fake Module: StarterGui.DANGER.FUNCTIONS
    local script = Instance.new("ModuleScript")
    script.Name = "FUNCTIONS"
    script.Parent = Converted["_DANGER"]
    local function module_script()
		...
		function FUNCTIONSmodule.createFloatingButton(item,button,buttonname)
			if not _G.DANGER.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", "")) then
		
		
				local newFloatingButton = _G.DANGER.FloatingButton:Clone()
				newFloatingButton.Parent = _G.DANGER.FloatingButtons
				...
				task.spawn(function()
					local buttontodestroy = _G.DANGER.FloatingButtons:FindFirstChild(string.gsub(buttonname, "_", ""))
		...
		function loader(module)
			...
			for _, item in ipairs(module) do
				...
				elseif item["Type"] == "Input" then
					local cloneinput = _G.DANGER.TextBoxPlaceholder:Clone()
				...
				elseif item["Type"] == "Toggle" then
					local clonetoggle = _G.DANGER.Toggle:Clone()
				...
				elseif item["Type"] == "Dropdown" then	
					local clonedropdown = _G.DANGER.Dropdown:Clone()
					local dropdownFrame = _G.DANGER.DropdownFrameSample
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				local success, err = pcall(function()
					local listbutton = _G.DANGER.ListButton:Clone()
				...
				if not success then
					warn("[DANGER] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
		...
		_G.DANGERFUNCTIONS = FUNCTIONSmodule
		return FUNCTIONSmodule
		
    end
    fake_module_scripts[script] = module_script
end
do -- Fake Module: StarterGui.DANGER.DraggableObject
    local script = Instance.new("ModuleScript")
...
do -- Fake Module: StarterGui.DANGER.ClickAndHold
    local script = Instance.new("ModuleScript")
...
do -- Fake Module: StarterGui.DANGER.Spring
    local script = Instance.new("ModuleScript")
    script.Name = "Spring"
    script.Parent = Converted["_DANGER"]
    local function module_script()
...
local function JQJVD_fake_script() -- Fake Script: StarterGui.DANGER.Open.InitOpen
...
local function QFRK_fake_script() -- Fake Script: StarterGui.DANGER.Open.OnClick
    local script = Instance.new("LocalScript")
    script.Name = "OnClick"
    script.Parent = Converted["_Open"]
    local req = require
	...
	script.Parent.MouseButton1Click:Connect(function()
		...
		if clickCount == 3 then
			...
			ts:Create(_G.DANGER.Menu, TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), 
...
local function KBEFORH_fake_script() -- Fake Script: StarterGui.DANGER.Open.Resizer
    local script = Instance.new("LocalScript")
    script.Name = "Resizer"
    script.Parent = Converted["_Open"]
    local req = require
...
local function VXLACE_fake_script() -- Fake Script: StarterGui.DANGER.Init
    local script = Instance.new("LocalScript")
    script.Name = "Init"
    script.Parent = Converted["_DANGER"]
    local req = require
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			print("[DANGER] - Using get_hidden_gui for anti-detection.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			print("[DANGER] - Using syn.protect_gui for anti-detection.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			print("[DANGER] - Using RobloxGui for anti-detection.")
		else
			warn("[DANGER] - Using CoreGui as anti-detection. This is the most basic coverage and can still be detected.")
		end
	end)
	
	print("[DANGER] - DANGER is now in " .. tostring(script.Parent:GetFullName()))
	if not s then
		warn("[DANGER] - Attempts at anti-detection failed. Using CoreGui as anti-detection.")
		warn(e)	
	end
	
	printidentity("[DANGER] - Your executor level (identity) is")
	
	local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
	print("[DANGER] - Your executor is " .. getExeName())
	...
	require(script.Parent.FUNCTIONS).notification("Welcome to DANGER Astralfire.")
...
local function JGXWB_fake_script() -- Fake Script: StarterGui.DANGER.FloatingButton.Keybinding
    local script = Instance.new("LocalScript")
...
local function MGTT_fake_script() -- Fake Script: StarterGui.DANGER.FloatingButton.Invisible
    local script = Instance.new("LocalScript")
...
local function TKTMNY_fake_script() -- Fake Script: StarterGui.DANGER.AddCustomModule.Add.LocalScript
    local script = Instance.new("LocalScript")
...
local function ZNJPL_fake_script() -- Fake Script: StarterGui.DANGER.AddCustomModule.Cancel.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function FHXBSSY_fake_script() -- Fake Script: StarterGui.DANGER.Menu.UIStroke.UIGradient.Animator
    local script = Instance.new("LocalScript")
...
local function PCGAKG_fake_script() -- Fake Script: StarterGui.DANGER.Menu.List.AutoSetup
    local script = Instance.new("LocalScript")
...
local function HXNHH_fake_script() -- Fake Script: StarterGui.DANGER.Menu.List.AddCustomModule.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function DGSNCYE_fake_script() -- Fake Script: StarterGui.DANGER.Menu.Close.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
	...
	script.Parent.MouseButton1Click:Connect(function()
		ts:Create(_G.DANGER.Menu, TweenInfo.new(0.4, Enum.EasingStyle.Circular, Enum.EasingDirection.Out), 
...
local function BVLUONR_fake_script() -- Fake Script: StarterGui.DANGER.Menu.CloseArea.CloseOpen
    local script = Instance.new("LocalScript")
...
local function TDXDSDS_fake_script() -- Fake Script: StarterGui.DANGER.Flee the Facility
    local script = Instance.new("LocalScript")
...
local function VHCKUS_fake_script() -- Fake Script: StarterGui.DANGER.Murder Mystery 2
    local script = Instance.new("LocalScript")
	...
	local fu = require(_G.DANGER.FUNCTIONS)
	...
	workspace.ChildRemoved:Connect(function(ch)
		if ch == getMap() and playerESP then
			...
			playerData = {}
			if _G.DANGER:FindFirstChild("AppliedMurdererBGUI") then _G.DANGER:FindFirstChild("AppliedMurdererBGUI"):Destroy() end
			if _G.DANGER:FindFirstChild("DGBGUIClone") then _G.DANGER:FindFirstChild("DGBGUIClone"):Destroy() end
			for _, v in ipairs(script.Parent:GetChildren()) do if v.Name == "PlayerESP" then v:Destroy() end end
		end
	end)
	...
	table.insert(module, {
		...
		Args = {2, {
			...
			Dropped_Gun = function()
				if gunDropESP then
					gunDropESP = false
					if _G.DANGER:FindFirstChild("GunESP") then _G.DANGER:FindFirstChild("GunESP"):Destroy() end
					if _G.DANGER:FindFirstChild("DGBGUIClone") then _G.DANGER:FindFirstChild("DGBGUIClone"):Destroy() end
				else
					gunDropESP = true
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[DANGER] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
...
local function COBWDKS_fake_script() -- Fake Script: StarterGui.DANGER.Universal
    local script = Instance.new("LocalScript")
	...
	table.insert(module, {
		Type = "Text",
		Args = {"Welcome to DANER! The open, free script hub."}
	})
	...
	table.insert(module, {
		...
		Args = {"Join our Discord", function(Self)
			if setclipboard then setclipboard("https://discord.gg/z4Z7kkJrHY") end
	...
	table.insert(module, {
		Type = "Button",
		Args = {"Hide DANGER", function(Self)
			if uis.KeyboardEnabled then
			...
			elseif uis.AccelerometerEnabled then
				...
				fu.notification("Can't hide DANGER!") -- how else are you gonna open???
			end
		end,}
	}
	)
	...
	table.insert(module, {
		...
		Args = {"Open developer console (debugging)", function(Self)
			...
			--_G.DANGER.Open.UIStroke.Transparency = 0
			--_G.DANGER.Open.TextTransparency = 0
			--ts:Create(_G.DANGER.Open, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
			--	Position = UDim2.fromScale(0.5, 0.903)
			--}):Play()

			--ts:Create(_G.DANGER.Open.UIStroke, TweenInfo.new(1), {
			--	Transparency = 1
			--}):Play()
			--ts:Create(_G.DANGER.Open, TweenInfo.new(1), {
			--	TextTransparency = 1
			--}):Play()
		end}
	}
	...
	local usernameColors = {
		["imjust_too"] = Color3.fromHex("#f48fff"),
		["imjust_t00"] = Color3.fromHex("#f48fff"),
		["dangerplus"] = Color3.fromHex("#f48fff"),
		["badgranpalex"] = Color3.fromHex("#ff0000"),
		["badgranpalexx"] = Color3.fromHex("#ff0000"),
		["2sniperss"] = Color3.fromHex("#0030ff"),
	}
	...
	local mainText = "DANGER Developer"
	local mainFont = Enum.Font.GothamBold
	local specialText = "DANGER+"
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"Hide DANGER+/Developer tags", function(Self, state)
